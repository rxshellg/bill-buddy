...
const Home = () => {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      if (wordCount < 4 || !hasRealContent) {
        setError(
          "Couldn't detect readable text. Please try a different image.",
        );
      } else {
  ...
  const handleUseExample = async () => {
    try {
      ...
      const exampleFile = new File([blob], "sampleReceipt.jpg", {
  ...
  const buttons = (
    <div className={styles.buttonOptions}>
      <button className="pinkButton" onClick={openModal}>
        Upload a receipt
      </button>
      <Link to="/Calculator">
        <button className="whiteButton">Split manually</button>
      </Link>
    </div>
  );
  ...
  return (
    <>
      <div className={isMobile ? styles.mobileHome : styles.desktopHome}>
        <div className={isMobile ? undefined : styles.content}>
          <div className={styles.textContainer}>
            <h1>Bill buddy</h1>
            <h2>
              For when the check comes and no one wants to think. Enter your
              numbers, split the bill, done.
            </h2>
            {!isMobile && buttons}
          </div>
      ...
      <Modal
        ...
        onHide={closeModal}
        backdrop={true}
        backdropClassName="upload-backdrop"
        centered
      >
        ...
        <Modal.Footer>
          ...
          >
            ...
            <button className="whiteButton" onClick={handleUseExample}>
              Use example image
            </button>
          </div>
        </Modal.Footer>
